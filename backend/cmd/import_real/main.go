...
func main() {
	...
	// 加载考纲 22 章（仅限该子科目下），建立章节名 -> 章节ID 的映射
	...
	log.Printf("已清空「%s - %s」下旧题目，开始按考纲 22 章重新归类导入",
...
// chapterKeywords 定义系统分析师综合知识 22 章的关键词，
// 用于把题目自动归类到对应章节。
// 排列规则：特异性强的知识域在前（如嵌入式、信息安全、新案例实践章），
// 泛化的知识域在后（如软件工程、项目管理），避免关键词误吸。
// 同组内顺序即匹配优先级（越靠前越优先）。
// 章节按《系统分析师教程（第 2 版 · 2024）》清华大学出版社目录。
var chapterKeywords = []struct {
	...
	{"信息物理系统", []string{"信息物理系统", "CPS", "工业互联网", "数字孪生", "智能工厂", "工业 4.0", "工业 4.0", "OPC UA", "MQTT", "时间敏感网络", "TSN"}},
	{"移动应用系统", []string{"移动应用", "Android", "iOS", "Flutter", "React Native", "Activity", "Service", "BroadcastReceiver", "Cocoa Touch", "移动端", "APP 开发", "APP 架构"}},
	{"大数据处理系统", []string{"大数据", "Hadoop", "HDFS", "MapReduce", "Spark", "Flink", "Storm", "Kafka Streams", "数据湖", "数据中台", "湖仓一体", "Hive", "HBase"}},
	{"微服务系统", []string{"微服务架构", "服务拆分", "服务治理", "服务网关", "API 网关", "服务注册", "服务发现", "Eureka", "Consul", "Nacos", "限流", "熔断", "Hystrix", "Sentinel", "Spring Cloud", "Service Mesh", "Istio", "分布式事务", "TCC", "Saga", "Seata"}},
	{"Web 应用系统", []string{"Web 应用", "前后端分离", "RESTful", "GraphQL", "Vue", "React", "Angular", "Spring", "Django", "Node.js", "PWA", "Service Worker"}},
	...
	{"数学与工程基础", []string{"线性规划", "单纯形", "对偶问题", "整数规划", "0-1 规划", "运输问题", "指派问题", "最大流", "最小费用流", "排队论", "泊松流", "M/M/1", "博弈论", "纳什均衡", "决策树", "盈亏平衡", "期望收益", "最小生成树", "Prim", "Kruskal", "最短路径 Dijkstra", "Dijkstra 算法", "Floyd 算法", "离散数学", "集合论", "关系矩阵", "命题逻辑", "谓词逻辑", "推理规则", "等价关系", "偏序", "群论", "环论", "概率论", "数学期望", "方差", "正态分布", "二项分布", "排列组合", "鸽巢原理", "容斥原理", "抽屉原理", "误差", "插值", "数值积分", "迭代法", "牛顿迭代", "矩阵运算", "行列式", "微积分", "线性代数", "特征值", "参数估计", "假设检验"}},
	{"计算机系统", []string{"哈夫曼", "邻接矩阵", "邻接表", "拓扑排序", "关键路径 AOE", "AOE 网", "时间复杂度", "空间复杂度", "二叉树", "平衡树", "红黑树", "线索二叉树", "顺序栈", "链栈", "共享栈", "循环队列", "双端队列", "优先队列", "队头", "队尾", "链表", "顺序表", "散列表", "冲突处理", "KMP", "模式串", "贪心策略", "动态规划", "分治法", "回溯法", "分支限界", "快速排序", "归并排序", "堆排序", "基数排序", "希尔排序", "直接插入排序", "折半查找", "顺序查找", "B 树", "B+ 树", "CPU", "运算器", "控制器", "寄存器", "指令周期", "指令流水线", "流水线吞吐率", "加速比", "Cache 命中率", "高速缓存", "主存", "辅存", "寻址方式", "立即寻址", "间接寻址", "总线仲裁", "冯·诺依曼", "哈佛结构", "RISC", "CISC", "Flynn 分类", "SISD", "SIMD", "MISD", "MIMD", "多核", "并行处理", "海明码", "CRC 校验", "原码", "反码", "补码", "移码", "浮点数表示", "规格化", "I/O 控制方式", "中断", "DMA", "通道", "可靠性 MTBF", "串联系统", "并联系统", "模冗余", "RAID", "冗余阵列", "磁盘阵列", "磁盘存取时间", "数据传输率", "字长", "机器字长", "操作系统", "进程状态", "进程调度", "进程同步", "进程互斥", "进程与线程", "进程 P", "子进程", "父进程", "线程", "协程", "死锁", "银行家算法", "信号量", "PV 操作", "PV操作", "pv操作", "P、V 操作", "管程", "临界资源", "临界区", "页面置换", "LRU", "FIFO 置换", "缺页中断", "虚拟存储器", "分页存储", "分段存储", "段页式", "磁盘调度", "SCAN", "SSTF", "索引节点", "位示图", "文件物理结构", "Spooling", "作业调度", "响应比高者优先", "时间片轮转", "多级反馈队列", "前趋图", "处理机系统", "单处理机", "文法", "正规式", "正则表达式", "词法分析", "语法分析", "语义分析", "中间代码", "目标代码", "编译器", "解释器", "传值调用", "引用调用", "地址传递", "递归调用", "闭包", "作用域", "动态绑定", "静态绑定", "多态的实现", "标识符", "表达式求值", "后缀表达式", "逆波兰式", "多媒体", "图像存储容量", "采样频率", "量化位数", "色深", "像素", "MPEG", "JPEG", "H.26", "音视频编码", "色彩空间", "帧率", "有损压缩", "无损压缩"}},
	{"计算机网络与分布式系统", []string{"分布式系统", "CAP 定理", "BASE 理论", "最终一致性", "Paxos", "Raft", "ZooKeeper", "一致性哈希", "数据分片", "消息队列", "发布订阅", "Kafka", "RabbitMQ", "负载均衡", "CDN 内容分发", "缓存穿透", "Redis 集群", "RPC 远程调用", "SOA 面向服务", "ESB 企业服务总线", "中间件", "容器编排", "Kubernetes", "区块链共识", "计算机网络", "OSI 参考模型", "TCP/IP 协议簇", "三次握手", "四次挥手", "滑动窗口", "拥塞控制", "IP 地址", "IP地址", "子网划分", "子网掩码", "网络地址", "CIDR", "无类域间路由", "IPv6", "ARP", "ICMP", "DHCP", "DNS 域名解析", "HTTP", "HTTPS", "FTP", "SMTP", "POP3", "SNMP 网络管理", "路由协议", "RIP", "OSPF", "BGP", "交换机", "VLAN", "以太网", "CSMA/CD", "令牌环", "网桥", "集线器", "拓扑结构", "星型", "环型", "总线型", "曼彻斯特编码", "差分曼彻斯特", "奈奎斯特定理", "香农定理", "波特率", "比特率", "多路复用", "时分复用", "频分复用", "ATM 异步传输", "路由器", "数据包", "IP 报文", "IP报文", "报文首部", "网段", "QoS", "IntServ", "DiffServ", "RSVP", "资源预留", "无线接入", "Wi-Fi", "5G 网络"}},
	...
	{"企业信息化", []string{"企业信息化", "信息化战略", "数字化转型", "ERP", "企业资源计划", "CRM 客户关系", "客户关系管理", "SCM 供应链", "供应链管理", "PLM 产品生命周期", "电子商务", "B2B", "B2C", "C2C", "O2O", "电子政务", "G2G", "G2B", "G2C", "智能制造", "两化融合", "业务流程重组", "业务流程再造", "BPR", "BPMN", "决策支持系统", "DSS", "商业智能", "BI 可视化", "知识管理", "显性知识", "隐性知识", "数据治理", "主数据", "元数据管理", "IT 服务", "ITIL", "ITSS", "服务级别协议", "SLA", "供应商关系管理", "企业应用集成", "EAI", "商业过程建模"}},
	...
	{"软件工程", []string{"软件工程", "软件开发模型", "瀑布模型", "原型模型", "增量模型", "螺旋模型", "喷泉模型", "敏捷开发", "Scrum", "极限编程 XP", "统一过程 RUP", "需求工程", "需求获取", "需求分析", "需求验证", "需求跟踪矩阵", "软件设计原则", "高内聚低耦合", "模块独立性", "结构化设计", "结构化方法", "结构化分析", "数据流图", "DFD", "数据字典", "白盒测试", "黑盒测试", "等价类划分", "边界值分析", "判定表", "因果图", "单元测试", "集成测试", "确认测试", "系统测试", "回归测试", "语句覆盖", "判定覆盖", "条件覆盖", "路径覆盖", "McCabe", "环形复杂度", "改正性维护", "适应性维护", "完善性维护", "预防性维护", "重构", "配置管理", "基线", "版本控制", "质量保证 QA", "质量控制 QC", "CMMI", "成熟度等级", "软件度量", "功能点估算", "COCCOMO 模型", "COCOMO 估算", "逆向工程", "软件再工程", "软件工程三要素", "软件工具", "开发方法", "面向数据流", "工作流参考模型", "服务建模", "面向服务架构", "系统分析阶段", "系统设计阶段", "Requirements validation", "systems analysis", "UML", "用例图", "类图", "对象图", "时序图", "顺序图", "通信图", "状态图", "活动图", "构件图", "部署图", "包图", "设计模式", "单例模式", "工厂方法", "抽象工厂", "观察者模式", "策略模式", "适配器模式", "装饰器模式", "代理模式", "外观模式", "组合模式", "桥接模式", "享元模式", "命令模式", "责任链", "中介者", "备忘录", "迭代器", "状态模式", "访问者", "封装", "继承", "多态", "SOLID", "开闭原则", "里氏替换", "依赖倒置", "接口隔离"}},
	{"系统规划与分析", []string{"系统规划", "可行性研究", "可行性分析", "技术可行性", "经济可行性", "运行可行性", "业务流程分析", "业务流程图", "系统分析报告", "总体规划", "信息化战略"}},
	{"软件需求工程", []string{"需求工程", "需求获取", "需求分类", "需求建模", "SRS", "需求规格说明", "需求评审", "需求验证", "需求跟踪矩阵", "需求变更", "联合需求计划", "JRP", "用例图"}},
	{"软件架构设计", []string{"软件架构", "架构风格", "分层架构", "管道-过滤器", "MVC", "微内核", "事件驱动", "SOA", "云原生", "架构 4+1 视图", "ADL", "ATAM", "SAAM", "CBAM", "架构评估", "EJB", ".NET Framework", "Spring"}},
	{"系统设计", []string{"概要设计", "详细设计", "模块设计", "接口设计", "数据结构设计", "程序流程图", "N-S 图", "PAD 图", "PDL", "设计评审"}},
	{"软件实现与测试", []string{"编码规范", "代码审查", "持续集成", "持续交付", "灰度发布", "CI/CD", "调试技术", "蛮力法", "回溯法", "原因排除法", "演绎法", "归纳法", "测试管理", "测试计划", "测试报告", "缺陷管理"}},
	{"系统运行与维护", []string{"系统运行", "运维管理", "ITIL", "ITSS", "服务级别协议", "事件管理", "系统维护", "改正性维护", "适应性维护", "完善性维护", "预防性维护", "系统评价", "技术债务", "再工程"}},
	{"系统分析师论文写作要点", []string{"论文摘要", "论文写作", "论题", "论点", "论据", "项目背景", "经验总结"}},
}
